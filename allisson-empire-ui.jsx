  ...
  return (
    <ModalOverlay onClose={onClose}>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
        <button style={styles.btnGold} onClick={onClose}>Launch Project ✦</button>
  ...
  return (
    <ModalOverlay onClose={onClose}>
      ...
      <div style={{ display: "flex", gap: 12 }}>
        <button style={styles.btnGold} onClick={onClose}>Dispatch Task ⚡</button>
        <button style={styles.btnGhost} onClick={onClose}>Cancel</button>
      </div>
    </ModalOverlay>
  );
...
function AgentResultsModal({ agent, onClose }) {
  const data = MOCK_AGENT_RESULTS[agent.id];
  if (!data) return null;
...
function RightSidebar() {
  const grouped = TOPICS.reduce((acc, t) => {
    const tasks = MOCK_LIVE_TASKS.filter(lt => lt.topic === t);
...
export default function AllissonEmpire() {
  ...
  const { displayed, done } = useTypingEffect(statusBrief);
